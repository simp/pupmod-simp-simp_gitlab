...
describe 'simp_gitlab using ldap' do
  ...
  context 'with TLS & PKI enabled' do
    shared_examples_for 'a web login for LDAP users' do |ldap_proto|
      ...
      # The acceptance criteria for this test is that a valid LDAP user can
      # authenticate to GitLab from a permitted client (via the OAuth
      # password-grant API, which delegates to LDAP).
      # ------------------------------------------------------------------------
      #
      # Troubleshooting:
      #
      ...
      #
      it 'permits a valid LDAP user to log in' do
        # BLOCKED on GitLab 19: the old HTML-form login scrape no longer works
        # (sign-in is rendered client-side), and the OAuth password grant that
        # would replace it was removed ("unsupported_grant_type"). Needs to be
        # reworked to a server-side gitlab-rails LDAP auth check.
        # See https://github.com/simp/pupmod-simp-simp_gitlab/issues/119
        skip('LDAP web login needs rework for GitLab 19 (no ROPC / client-side form) -- see issue #119')
      end
