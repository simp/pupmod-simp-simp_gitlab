...
describe 'change_gitlab_root_password misc' do
  ...
  context 'error cases' do
    it 'fails when the gitlab-rails command is missing' do
    ...
    it 'fails when the gitlab-ctl command is missing' do
    ...
    it 'fails when the GitLab postgresql process is not running' do
    ...
    it 'fails when the gitlab-rails console load times out' do
      cmd = "#{exe} -t 1 #{password}"
      on(gitlab_server, cmd, acceptable_exit_codes: [1])
    end

    it 'fails when no password is specified' do
      on(gitlab_server, exe, acceptable_exit_codes: [1])
    end

    it 'fails when the password is empty' do
      cmd = "#{exe} ''"
      on(gitlab_server, cmd, acceptable_exit_codes: [1])
    end
  end
